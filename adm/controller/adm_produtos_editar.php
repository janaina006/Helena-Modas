...
if(isset($_POST['btn_apagar'])){
    if($gravar->Apagar($_POST['pro_id_apagar'])){
        echo '<div class="alert alert-success">Produto
        Deletado com sucesso!!</div>';
        @unlink($_POST['pro_img_arquivo']);
        Rotas::Redirecionar(2, Rotas::pag_ProdutosADM());
        exit();
    }else{
        echo '<div class="alert alert-danger">Produto
        não foi excluido com sucesso!!</div>';
    }
}
...
if(isset($_POST['pro_nome']) && isset($_POST['pro_valor'])){
    ...
    if($gravar->Alterar($pro_id)){
        ...
        echo '<div class="alert alert-danger">Erro ao editar o produto!!'; 
